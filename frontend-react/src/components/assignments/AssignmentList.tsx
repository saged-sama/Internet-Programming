...
export default function AssignmentList({ assignments, onViewDetails }: AssignmentListProps) {
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {assignments.map((assignment) => {
        ...
        return (
          <div 
            key={assignment.id} 
            className="bg-card rounded-lg shadow-sm overflow-hidden border border-muted"
          >
            <div className="p-4">
              ...
              <div className="flex justify-between items-center text-sm">
                <div className={`${isDeadlinePassed ? 'text-red-600' : 'text-muted-foreground'}`}>
                  Deadline: {format(new Date(assignment.deadline), 'MMM dd, yyyy hh:mm a')}
                </div>
